...
class DocumentEncoder:
    ...
    def encode_tokens_chunk(self, chunk, max_chunk_size, bert_model_dimension=512):
        ...
        assert len(chunk_token_ids) <= bert_model_dimension, f"chunk_token_ids {len(chunk_token_ids)} longer than {bert_model_dimension}"
        if len(chunk_token_ids) > max_chunk_size: print(f"Chunk has now {len(chunk_token_ids)} elements")
        # convert to numpy array
        chunk_token_ids = np.array(chunk_token_ids)

        # padding
        num_unused_positions = bert_model_dimension - len(chunk_token_ids)
    ...
    def encode_entity_positions_sentence(self, sentence):
        # TODO new tokens have to be updated -> mapping again???
        ...
        if self._use_slot_labels:
            ...
            entities = [entity for entity in sentence.get_entities() if len(
                entity.get_referencing_slot_names()) > 0]
        else:
    ...
    def encode_entity_positions_document_chunking(self, document_chunking, dummy_value=0):
        ...
        bert_model_dim = document_chunking.get_bert_dimension()
        ...
        for chunk in document_chunking.get_chunks():
            ...
            assert len(encoded_start_positions_chunk) == len(encoded_end_positions_chunk), "start and end positions not same length"
            assert len(encoded_start_positions_chunk) <= bert_model_dim, f"start/end positions {len(encoded_start_positions_chunk)}longer than {bert_model_dim}"

            # padding
            num_unused_tokens = bert_model_dim - len(encoded_start_positions_chunk)
    ...
    def create_token_masks_document_chunking(self, document_chunking):
        token_masks = []
        max_chunk_size = document_chunking.get_bert_dimension()
