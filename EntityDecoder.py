...
class EntityDecoder:
    ...
    def decode_document_chunking(self, encoded_start_positions, encoded_end_positions, doc_chunking):
        ...
        # if only one chunk, squeeze flattens the list so that encoded_start_positions[i] accesses only the first single elem
        else:
            for i, chunk in enumerate(doc_chunking.get_chunks()):
                entities_chunk = self.decode_chunk(encoded_start_positions, encoded_end_positions, chunk)
                decoded_entities.extend(entities_chunk)
